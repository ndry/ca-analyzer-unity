...
public class RuleConverter {
    ...
    public static int[] GetTableFromTotalistic(int[] table, int stateCount)
        => ConvertTable(
            table,
            (n1, c, n2, _, __) => n1 + c + n2,
            stateCount);

    public static int[] GetTableFromTotalistic(BigInteger code, int stateCount) {
        var sizePower = (stateCount - 1) * 3 + 1;
        return GetTableFromTotalistic(
            Rule.GetTableFromCode(code, sizePower, stateCount),
            stateCount);
    }
    public static int[] GetTableFromLegacy(int[] table, int stateCount)
    => ConvertTable(table, (n1, c, n2, pc, _) => {
        var combinedState = 0;
    ...
    public static int[] GetTableFromLegacy(BigInteger code, int stateCount) {
        var sizePower = Rule.GetRuleSpaceSizePower(stateCount);
        var t = Rule.GetTableFromCode(code, sizePower, stateCount);
        Array.Reverse(t);
        return GetTableFromLegacy(t, stateCount);
    }
    ...
    public static int[] RepackTableForNCell(Packer packer, int[] table, int n) {
        ...
        for (var stc = 0; stc < t2.Length; stc++) {
            ...
            for (var i = 0; i < n; i++) {
                ...
                var stci = Rule.GetCombinedState(n1cn2, pc, stateCount);
                var sti = table[stci];
